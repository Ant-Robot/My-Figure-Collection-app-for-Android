...
public class GLToolbox
{
	...
	public static int loadShader(final int shaderType, final String source)
	{
		...
		if (shader != 0)
		{
			...
			if (compiled[0] == 0)
			{
				final String info = GLES20.glGetShaderInfoLog(shader);
	...
	{
		final int vertexShader = GLToolbox.loadShader(GLES20.GL_VERTEX_SHADER, vertexSource);
		if (vertexShader == 0) return 0;
		final int pixelShader = GLToolbox.loadShader(GLES20.GL_FRAGMENT_SHADER, fragmentSource);
		...
		if (program != 0)
		{
			...
			{
				final String info = GLES20.glGetProgramInfoLog(program);
