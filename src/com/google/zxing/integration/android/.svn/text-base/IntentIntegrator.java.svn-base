 ...
 * result, without any need to integrate, modify, or learn the project's source code.
 * </p>
 * 
 ...
 * 
 * <p>
 * There are a few steps to using this integration. First, your {@link Activity} must implement the method {@link Activity#onActivityResult(int, int, Intent)}
 * and include a line of code like this:
 * </p>
 * 
 * <p>
 * {@code public void onActivityResult(int requestCode, int resultCode, Intent
 * intent) IntentResult scanResult =
 * IntentIntegrator.parseActivityResult(requestCode, resultCode, intent); if
 * 
 * 
 * (scanResult != null) // handle scan result } // else continue with any other code you need in the method ... } }
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * You can use {@link #initiateScan(Activity, CharSequence, CharSequence, CharSequence, CharSequence)} or {@link #initiateScan(Activity, int, int, int, int)} to
 * customize the download prompt with different text labels.
 * </p>
 * 
 * <p>
 * Note that {@link #initiateScan(Activity)} returns an {@link AlertDialog} which is non-null if the user was prompted to download the application. This lets
 * the calling app potentially manage the dialog. In particular, ideally, the app dismisses the dialog if it's still active in its {@link Activity#onPause()}
 * method.
 * </p>
 * 
...
public final class IntentIntegrator {
    ...
    public static final int    REQUEST_CODE        = 222;                                                                       // get it?
    ...
    public static final String JAPANESE_CODE_TYPES = "EAN_8,EAN_13";
    ...
    /**
     * See {@link #initiateScan(Activity, CharSequence, CharSequence, CharSequence, CharSequence)} -- same, but takes string IDs which refer to the
     * {@link Activity}'s
    ...
    public static AlertDialog initiateScan(Activity activity, int stringTitle, int stringMessage, int stringButtonYes, int stringButtonNo) {
        return initiateScan(activity, activity.getString(stringTitle), activity.getString(stringMessage), activity.getString(stringButtonYes),
                            activity.getString(stringButtonNo));
    }

    /**
     * See {@link #initiateScan(Activity, CharSequence, CharSequence, CharSequence, CharSequence, CharSequence)} -- same, but scans for all supported barcode
     * types.
     * 
     * @param stringTitle
    ...
    public static AlertDialog initiateScan(Activity activity,
        ...
        return initiateScan(activity, stringTitle, stringMessage, stringButtonYes, stringButtonNo, JAPANESE_CODE_TYPES);
    }
     ...
     * </p>
     * 
     * @return null if the event handled here was not related to {@link IntentIntegrator}, or else an {@link IntentResult} containing the result of the scan. If
     *         the user cancelled
     *         scanning, the fields will be null.
     */
    ...
    /**
     * See {@link #shareText(Activity, CharSequence, CharSequence, CharSequence, CharSequence, CharSequence)} -- same, but takes string IDs which refer to the
     * {@link Activity}'s
     * resource bundle entries.
     */
    public static void shareText(Activity activity, CharSequence text, int stringTitle, int stringMessage, int stringButtonYes, int stringButtonNo) {
        shareText(activity, text, activity.getString(stringTitle), activity.getString(stringMessage), activity.getString(stringButtonYes),
                  activity.getString(stringButtonNo));
    }
